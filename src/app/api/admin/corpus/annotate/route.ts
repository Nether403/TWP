 ...
 * Used by the HCC annotation interface. Requires authenticated admin role.
 */
export async function PUT(request: NextRequest) {
  try {
    ...
    // Audit log — real reviewer identity
 ...
 * Used by the Corpus Manager's annotation panel. Requires authenticated admin role.
 */
  ...
  try {
    ...
    if (!testimonyId || !annotations) {
      return NextResponse.json(
        { error: "testimonyId and annotations required" },
        { status: 400 }
      );
    }
    ...
    await supabaseAdmin.from("audit_log").insert({
      ...
      metadata: {
        ...
        tags_count:
          (annotations.cap_tags?.length || 0) +
          (annotations.rel_tags?.length || 0) +
          (annotations.felt_tags?.length || 0),
      },
    });
